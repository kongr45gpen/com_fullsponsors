 ...
 */
class ManagedsponsorsTableSponsor extends JTable
{
	/**
	...
	{
		parent::__construct('#__managedsponsors_sponsor', 'id', $db); 
	}	
	...
	/**
	 * Overload the store method for the Sponsor table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New sponsor. A sponsor created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('sponsor', 'ManagedsponsorsTable');

			if ($table->load(array('alias' => $this->alias, 'catid' => $this->catid)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_MANAGEDSPONSORS_SPONSOR_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_managedsponsors.sponsor.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_managedsponsors.sponsor.'.(int) $this->$k;
	}
	...
	protected function _getAssetParentId(JTable $table = NULL, $id = NULL) 
	{
		...
		$asset->loadByName('com_managedsponsors');
