 ...
 * Sponsors Model
 */
class ManagedsponsorsModelSponsors extends JModelList
{
	...
	{ 
		...
		if (ManagedsponsorsHelper::checkArray($items))
		{
			...
			{
				$access = (JFactory::getUser()->authorise('sponsor.access', 'com_managedsponsors.sponsor.' . (int) $item->id) && JFactory::getUser()->authorise('sponsor.access', 'com_managedsponsors'));
				if (!$access)
				{
					unset($items[$nr]);
					continue;
	...
	protected function getListQuery()
	{
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('(a.name LIKE '.$search.' OR a.website LIKE '.$search.')');
			}
		}
